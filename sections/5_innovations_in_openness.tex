...
By explicitly using public\footnote{Some data for the draftquality model is restricted, and access requires a small amount of paperwork.} APIs and data and releasing our utilities and Makefile source code under an open license (MIT), we have implemented a turn-key process for replicating our model-building and evaluation pipeline.  A developer can review this pipeline for issues, and be certain that they aren't missing a step of the process because all steps are captured in the Makefile.  They can also build on the process (e.g. add new features) incrementally and restart the pipeline.  In our own experience, this explicit pipeline is extremely useful for debugging and for making incremental improvements to ORES's models.

The simplest entry point to our Makefile is for a user to \texttt{make models}, rebuilding all the models in the repository.  We regularly perform this process ourselves to ensure that the Makefile is an accurate representation of the data flow pipeline.  Performing complete rebuild is essential when a breaking change is made to one of our libraries.  The resulting serialized models are saved to the source code repository so that a developer can review the history of any specific model and even experiment with generating scores using old model versions.
